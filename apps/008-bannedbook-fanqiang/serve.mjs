...
const port = Number.parseInt(process.env.FANQIANG_ATLAS_PORT || '4208', 10);
...
server.on('error', (error) => {
  if (error.code === 'EADDRINUSE') {
    console.error(`端口 ${port} 已被占用；可设置 FANQIANG_ATLAS_PORT 后重试。`);
  } else {
    console.error(error);
  }
  process.exitCode = 1;
...
server.listen(port, host, () => {
  console.log(`fanqiang Capability Atlas：http://${host}:${port}/`);
