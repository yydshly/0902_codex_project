...
在 `catalog/projects.json` 的 `projects` 数组末尾新增条目。数组必须严格按 `id` 递增，路径必须指向真实文件。可选的 `cover` 与 `demo` 字段只有在相应素材或产物存在时才填写。
...
同时把目录根节点的 `updatedAt` 更新为本次变更日期。若暂时没有合规封面或 Demo，直接省略相应字段，不要使用虚构占位路径。
