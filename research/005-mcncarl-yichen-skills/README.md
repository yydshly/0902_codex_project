...
| 问题 | 我们的判断 |
| --- | --- |
| 它的本质是什么？ | 个人日常工作经验的 Skill 化档案，由 22 个 Skill、引用资料、脚本、测试和插件组成。 |
| 它是不是 Agent？ | 不是。Codex、Claude Code 或 ChatGPT 等宿主 Agent 负责理解目标、选择能力和综合结果。 |
| 底层如何工作？ | `SKILL.md` 提供触发、SOP 与边界；脚本、浏览器、CLI、API 或 MCP 执行；manifest、hash、回读和状态机提供完成证据。 |
| 最值得借鉴什么？ | 把隐性经验变成可发现、可交接、可授权、可验证的工作协议，而不是照搬作者的账号、平台和个人环境。 |
| 我们应该怎样使用？ | 遇到真实任务再查找对应 Skill；优先借鉴方法，必要时在合规的个人非商业、低风险环境体验，并记录结果。 |
| 什么时候沉淀为自己的 Skill？ | 同类任务反复出现、收益可衡量、边界可说明、完成可证明后，再独立重写；证据不足就停止。 |
| 是否需要继续全量深入？ | 不需要。无需试遍 22 项能力，当前研究已经足够作为能力索引、场景参考和按需试用入口。 |
...
Demo 把 22 项能力、四层架构、实现成熟度、依赖边界和团队采用路线放在同一条可交互阅读路径里。每个 Skill 都有典型场景；流程模拟器进一步用“内容创作、深度研究、私域资产”三类案例展示 Agent 决策、Skill 接力、handoff、安全闸门和最终成果。

Revision 3 增加了面向我们自己的采用闭环：22 项 Skill 被逐项归入“方法借鉴 / 个人试用 / 授权后重写 / 暂不采用”；浏览器本地台账可记录一次真实日常任务的时间、人工介入、产物、问题和效果，并生成统计、JSON 导出与删除操作；七段式 Skill 设计卡和四道升级判断门则说明何时值得把反复验证过的做法重写为我们的能力。流程样例仍是说明性的，台账内容才来自用户真实填写；两者都不会运行上游 Skill、读取账号登录态或访问私人数据。

Revision 4 将研究收束为“按需使用”模式：页面新增阶段性最终理解，原先按周推进的建设路线改为“任务出现再查 → 低风险体验 → 多次有效再沉淀”，并明确无需为了覆盖率试遍全部 22 个 Skill。
...
- `2026-09-04`：收束阶段性最终理解，将后续路线调整为按需查阅、低风险体验和证据驱动的独立沉淀。
- `2026-09-04`：新增 22 项采用决策矩阵、浏览器本地真实试用台账、JSON 导出、四道升级判断门和七段式自有 Skill 模板。
